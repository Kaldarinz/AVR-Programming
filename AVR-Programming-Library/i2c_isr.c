...
uint8_t cur_trans_data_index = 0;
...
{
    if (cur_trans_data_index<trans_data_len)
    {
        // Load next byte
        TWDR = trans_data[cur_trans_data_index];
        // Increment data index
        cur_trans_data_index++;
        // Send data
        TWCR |= (1<<TWINT);
    }
...
void i2c_choose_slave(void)
{
    ...
    if ( (mode == MODE_MT)&&(addr!=ADDR_NOT_SET) )
        {
            // Load address
            TWDR = (addr<<1);
    ...
    else if ( (mode == MODE_MR)&&(addr!=ADDR_NOT_SET) )
        {
...
void i2c_master_init(unsigned long frequency)
{
    ...
    TWBR = (uint8_t)((((F_CPU * (1 << (prescaler + 1) ) / frequency) / I2C_SCL_FREQUENCY_PRESCALER) - 16 ) / 2); // Set bitrate
}
